		...
		<el-dialog :close-on-click-modal="false" :close-on-press-escape="false" title="输入支付密码" v-if="zfqrk" :visible.sync="zfqrk"
		 :modal="false" width="500px">
			...
			<el-form label-width="200px">
				...
				<el-form-item v-if="chaxuntype==0" label="订单金额: ">
					<span v-if="tiaoshu.weizhang">0.00元</span>
					<span v-else>{{chaxunres.pay_num}}元</span>
				</el-form-item>
				<el-form-item v-if="chaxuntype==1" label="订单金额: ">
					<span v-if="tiaoshu.weibao">0.00元</span>
					<span v-else>{{chaxunres.pay_num}}元</span>
				</el-form-item>
				<el-form-item v-if="chaxuntype==2" label="订单金额: ">
					<span v-if="tiaoshu.zonghe">0.00元</span>
					<span v-else>{{chaxunres.pay_num}}元</span>
				</el-form-item>
				...
				<el-form-item label="">
					<!-- <el-input v-model="pay.paypassword" ></el-input> -->
					<span v-if="chaxuntype==0">您当前剩余{{tiaoshu.weizhang}}条查询次数</span>
					<span v-if="chaxuntype==1">您当前剩余{{tiaoshu.weibao}}条查询次数</span>
					<span v-if="chaxuntype==2">您当前剩余{{tiaoshu.zonghe}}条查询次数</span>
				</el-form-item>
			</el-form>
				...
				<div class="car-box">
					<div class="fLeft">
						<div class="opic_group">
							...
							<div class="opic_bbox">
								<img v-if="cartxt.car_pic_url[0]" :src="cartxt.car_pic_url[0]" alt="暂无图片">
								<!-- <img v-if="!cartxt.car_pic_url[0]&&zw" src="../assets/images/zanwutupian.png" > -->
								<img v-if="cartxt.car_pic_url==''&&cartxt.check_report_path==''" src="../assets/images/无图无检.jpg" class="image2"
								 alt="暂无图片">
								<img v-if="cartxt.car_pic_url==''&&cartxt.check_report_path!=''" src="../assets/images/无图有检.jpg" class="image2"
								 alt="暂无图片">
					...
					<div class="cardetail">
						...
						<ul>
						  ...
						  -->
							...
							<li><img src="../assets/images/detail/paifnag.png" width="32" height="33"><span ref="li4">{{cartxt.catid_text}}</span>车辆类型</li>
    ...
    font-weight: 700;" v-if="cartxt.is_vip=='1'">会员拍:本车需开通会员参拍！</span>
				...
				<div class="cx-tabs-both">
					...
					<div class="cx-n-box">
						...
						<div class="cn-note" v-html="cartxt.session_desc">
							<!-- {{cartxt.session_desc}} -->
					...
					<div class="cx-n-box">
						...
						<div class="cn-table" v-if="cartxt.cheliangleixing=='1'">
							<table cellpadding="0" cellspacing="1">
								<tbody>
									<tr>
										...
										<td style="background-color: rgb(239, 239, 239);">车辆品牌</td>
							...
							<table cellpadding="0" cellspacing="1">
								<tbody>
									...
									<tr>
										<td style="background-color: rgb(239, 239, 239);">燃油</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.ranyouleixing}}</td>
										<td style="background-color: rgb(239, 239, 239);">发动机品牌</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.fadongjipinpai}}</td>
										<td style="background-color: rgb(239, 239, 239);">排放标准</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.paifangbiaozhun}}</td>
									</tr>
									<tr>
										<td style="background-color: rgb(239, 239, 239);">驱动马力</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.zuidamali}}</td>
										<td style="background-color: rgb(239, 239, 239);">入库时间</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.rukushijian}}</td>
										<td style="background-color: rgb(239, 239, 239);">停车费/天</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.tingchefeimeitian}}</td>
									</tr>
							...
							<table cellpadding="0" cellspacing="1">
								<tbody>
									...
									<tr>
										<td style="background-color: rgb(239, 239, 239);">户籍性质</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.hujixingzhi}}</td>
										<td style="background-color: rgb(239, 239, 239);">表显里程</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.mileage_number}}</td>
										<td style="background-color: rgb(239, 239, 239);">上牌时间</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.get_card_time}}</td>
									</tr>
									<tr>
										<td style="background-color: rgb(239, 239, 239);">出厂日期</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.chuchangriqi}}</td>
										<td style="background-color: rgb(239, 239, 239);">年检到期</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.nianjiandaoqi}}</td>
										<td style="background-color: rgb(239, 239, 239);">交强险到期</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.jiaoqiangxiandaoqishijian}}</td>
									</tr>
									<tr>
										<td style="background-color: rgb(239, 239, 239);">货箱长</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.chang}}</td>
										<td style="background-color: rgb(239, 239, 239);">货箱宽</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.kuan}}</td>
										<td style="background-color: rgb(239, 239, 239);">货箱高</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.gao}}</td>
									</tr>
							...
							<table cellpadding="0" cellspacing="1">
								<tbody>
									<tr>
										...
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.catid_text}}</td>
						...
						<div class="cn-table" v-if="cartxt.cheliangleixing=='2'">
							<table cellpadding="0" cellspacing="1">
								<tbody>									
									<tr>
										<td style="background-color: rgb(239, 239, 239);">车辆编号</td>
									...
									<tr>						
										<td style="background-color: rgb(239, 239, 239);">挂车车牌</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.guachechejiahao}}</td>
										<td style="background-color: rgb(239, 239, 239);">挂车车架</td>
										<td style="background-color: rgb(255, 255, 255);">
											<font>{{cartxt.guachechejia}}</font>
										</td>
										<td style="background-color: rgb(239, 239, 239);"> 车辆类型</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.catid_text}}
										</td>					
									</tr>
								</tbody>
							</table>
							...
							<table cellpadding="0" cellspacing="1">
								<tbody>
									<tr>
										<td style="background-color: rgb(239, 239, 239);">上牌时间</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.get_card_time}}</td>
										<td style="background-color: rgb(239, 239, 239);">出场日期</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.chuchangriqi}}</td>
										<td style="background-color: rgb(239, 239, 239);">年检到期</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.nianjiandaoqi}}</td>
									</tr>
									<tr>
										<td style="background-color: rgb(239, 239, 239);">长</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.chang}}</td>
										<td style="background-color: rgb(239, 239, 239);">宽</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.kuan}}</td>
										<td style="background-color: rgb(239, 239, 239);">高</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.gao}}</td>
									</tr>
								</tbody>
							</table>
							...
							<table cellpadding="0" cellspacing="1">
								<tbody>
									<tr>
										<td style="background-color: rgb(239, 239, 239);">容积</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.rongji}}</td>
										<td style="background-color: rgb(239, 239, 239);">轴数</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.zhoushu}}</td>
									</tr>
									<tr>
										<td style="background-color: rgb(239, 239, 239);">是否过户</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.is_change}}</td>
						...
						<div class="cn-table" v-if="cartxt.cheliangleixing=='3'">
							<table cellpadding="0" cellspacing="1">
								<tbody>									
									<tr>
										<td style="background-color: rgb(239, 239, 239);">车辆编号</td>
									...
									<tr>						
										<td style="background-color: rgb(239, 239, 239);">车牌号</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.car_number}}</td>
										<td style="background-color: rgb(239, 239, 239);">挂车牌号</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.guachechepai}}</td>
										<td style="background-color: rgb(239, 239, 239);">车辆类型</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.catid_text}}
										</td>					
									</tr>
							...
							<table cellpadding="0" cellspacing="1">
								<tbody>
									<tr>
										<td style="background-color: rgb(239, 239, 239);">车辆品牌(车头)</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.brand_info}}</td>
										<td style="background-color: rgb(239, 239, 239);">车辆品牌(挂车)</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.guachepinpai}}</td>
										<td style="background-color: rgb(239, 239, 239);">发动机号</td>
										<td style="background-color: rgb(255, 255, 255);"><font>{{cartxt.engine_number}}</font></td>
									</tr>
									<tr>
										<td style="background-color: rgb(239, 239, 239);">车辆型号</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.cheliangxinghao}}</td>
										<td style="background-color: rgb(239, 239, 239);">驱动形式</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.qudongxingshi}}</td>
										<td style="background-color: rgb(239, 239, 239);">最大马力</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.zuidamali}}</td>
									</tr>
								</tbody>
							</table>
							...
							<table cellpadding="0" cellspacing="1">
								<tbody>
									<tr>
										...
										<td style="background-color: rgb(239, 239, 239);">排放标准</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.paifangbiaozhun}}</td>
										<td style="background-color: rgb(239, 239, 239);">燃油类型</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.ranyouleixing}}</td>
									</tr>
									<tr>
										<td style="background-color: rgb(239, 239, 239);">变速箱品牌</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.biansuxiangpinpai}}</td>
										<td style="background-color: rgb(239, 239, 239);">变速箱档位</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.biansuxiangdangwei}}</td>
										<td style="background-color: rgb(239, 239, 239);">换挡方式</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.huandangfangshi}}</td>
									</tr>
								</tbody>
							</table>
							...
							<table cellpadding="0" cellspacing="1">
								<tbody>
									<tr>
										<td style="background-color: rgb(239, 239, 239);">是否液力缓速器</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.shifouyelihuansuqi}}</td>
										<td style="background-color: rgb(239, 239, 239);">后桥规格</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.houqiaoguige}}</td>
										<td style="background-color: rgb(239, 239, 239);">轮胎规格</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.luntaiguige}}</td>
									</tr>
									<tr>
										
										<td style="background-color: rgb(239, 239, 239);">提车地</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.car_place_info}}</td>
										<td style="background-color: rgb(239, 239, 239);">车架号(头)</td>
										<td style="background-color: rgb(255, 255, 255);">
										<font>{{cartxt.frame_number}}</font></td>
										<td style="background-color: rgb(239, 239, 239);">车架号(挂)</td>
										<td style="background-color: rgb(255, 255, 255);"><font>{{cartxt.guachechejiahao}}</font></td>
									</tr>
								</tbody>
							</table>	
							...
							<table cellpadding="0" cellspacing="1">
								<tbody>
									<tr>
										<td style="background-color: rgb(239, 239, 239);">箱货(长)</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.chang}}</td>
										<td style="background-color: rgb(239, 239, 239);">箱货(宽)</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.kuan}}</td>
										<td style="background-color: rgb(239, 239, 239);">箱货(高)</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.gao}}</td>
									</tr>
									<tr>
							...
							<table cellpadding="0" cellspacing="1">
								<tbody>
									<tr>
										<td style="background-color: rgb(239, 239, 239);">年检到期时间</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.nianjiandaoqi}}</td>
										<td style="background-color: rgb(239, 239, 239);">保险到期时间(交强)</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.jiaoqiangxiandaoqishijian}}</td>
										<td style="background-color: rgb(239, 239, 239);">保险到期时间(商业)</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.shangyexiandaoqishijian}}</td>
									</tr>
									<tr>					
										<td style="background-color: rgb(239, 239, 239);">钥匙</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.is_key}}</td>
										<td style="background-color: rgb(239, 239, 239);">行驶证</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.is_driving_license}}</td>
										<td style="background-color: rgb(239, 239, 239);">是否正常启动</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.is_normal}}</td>
						...
						<div class="cn-table" v-if="cartxt.catid=='1'">
							<table cellpadding="0" cellspacing="1">
								<tbody>									
									<tr>
										<td style="background-color: rgb(239, 239, 239);">车辆编号</td>
							...
							<table cellpadding="0" cellspacing="1">
								<tbody>
									<tr>
										<td style="background-color: rgb(239, 239, 239);">排量</td>
									...
									<tr>
										<td style="background-color: rgb(239, 239, 239);">能源类型</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.ranyouleixing}}</td>
										<td style="background-color: rgb(239, 239, 239);">入库时间</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.rukushijian}}</td>
										<td style="background-color: rgb(239, 239, 239);">停车费/天</td>
							...
							<table cellpadding="0" cellspacing="1">
								<tbody>
									<tr>
										<td style="background-color: rgb(239, 239, 239);">车辆性质</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.cheliangxingzhi}}</td>
										<td style="background-color: rgb(239, 239, 239);">户籍性质</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.hujixingzhi}}</td>
										<td style="background-color: rgb(239, 239, 239);">出厂日期</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.chuchangriqi}}</td>
									...
									<tr>
										<td style="background-color: rgb(239, 239, 239);">年检到期</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.nianjiandaoqi}}</td>
										<td style="background-color: rgb(239, 239, 239);">保险到期</td>
										<td style="background-color: rgb(255, 255, 255);">{{cartxt.baoxiandaoqi}}</td>
						...
						<div class="cn-table" v-if="(cartxt.cheliangleixing==null||cartxt.cheliangleixing=='')&&cartxt.catid!='1'">
							<table cellpadding="0" cellspacing="1">
								<tbody>
									<!-- <tr>
					...
					<!-- 交付资料 -->
					<div class="cx-n-box" v-if="cartxt.catid!='1'">
						<div class="cn-tit">
							<b>交付资料</b>
						</div>
					...
					<div class="cx-n-box">
						...
						<div class="cn-images">
							...
							<viewer :images="cartxt.car_pic_url">
								<div class="banner__wrapper" style="margin-bottom: 5px;" v-for="(item,index) in cartxt.car_pic_url" :key="index">
			...
			<div class="content-right">
				...
				<div class="content-right_Same">
					...
					<div class="content-right_ser_titie_form">
						...
						<!-- <div class="right_top_sc_item" v-for="item in withAuction" :key="item.id" @click="goAuctiondetail(item.id)">

							<div class="sc_item_img" style="float: left;">
								<img :src="item.car_pic_url" alt="" style="width: 100%;height: 100%;">
							</div>
							<div class="sc_item_txt" style="float: left;">
								<p style="width: 169px;height: 54px;overflow: hidden;">{{item.title}}</p>
								<p style="margin-top: 15px;">{{item.start_price}}|{{item.car_number}}</p>
							</div>
						...
						<!-- <div class="gengduo" @click="goAuction">更多>></div> -->
					</div>
				</div>
		...
		methods: {
			...
			getcartext() {
				...
				this.$api.carDetail({
					...
					if (this.cartxt.catid_text.length > 8) {
						this.$refs.li4.style = "font-size: 2px;line-height: 15px;"
						// this.$refs.li4.style.fontSize="12px"
					}
					this.zw = true;
					this.seth();
					this.getcarlist();
			...
			weibao(type) {
				...
				this.$api.getFuwuIndent({
					...
					if (res.code == 1) {
						...
						this.tiaoshu = res.data.user_level_special
						console.log(this.tiaoshu)
						this.chaxuntype = type
