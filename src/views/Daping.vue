<template>
	<div class="box">
				...
				<div class="jiage">
					...
					<div class="y">
						<div class="y_top">
							...
							<span v-else><img src="../assets/xcp/xcp_10.gif" >当前最高价</span>
						...
						<div class="y_">
							<span class="y_1" v-show="cardata.car.max_cost==''||cardata.car.max_cost==null">{{parseFloat(cardata.car.start_price)/10000}}</span>
							<span class="y_1" ref="jiage" v-show="cardata.car.max_cost!=''&& cardata.car.max_cost!=null">{{cardata.car.max_cost}}</span>
							...
							<span class="y_2" ref="jiage2">万</span>
						</div>
						<div class="mob" ref="mob">
							<div v-if="mobile!=''"><img src="../assets/xcp/现场拍_1.gif" style="vertical-align:middle;width: 7%;"><span style="vertical-align:middle;">{{mobile}}</span></div>
							<div v-if="mobile2!=''"><img src="../assets/xcp/现场拍_2.gif"  style="vertical-align:middle;width: 7%;"><span style="vertical-align:middle;">{{mobile2}}</span></div>
							<div v-if="mobile3!=''"><img src="../assets/xcp/现场拍_3.png"  style="vertical-align:middle;width: 7%;"><span style="vertical-align:middle;">{{mobile3}}</span></div>
						</div>
					</div>
				</div>
		...
		<div class="xia"  v-if="is">
			<div class="xia_1">
				<div class="xia_1_1">
					<div class="xia_1_1_1">
						<span>车辆名称：</span>
					</div>
					<span class="xia_1_1_2" style="color:#d92e24 ;">
						<!-- {{cardata.car.brand_info}}&nbsp;&nbsp;{{cardata.car.title}} -->
						{{cardata.car.title}}
					</span>
				</div>
				<div class="xia_1_2"></div>
				<div class="xia_1_3">
					<div class="xia_1_3_1">
						<div class="xia_1_3_1_1">
							车牌号：<span>{{cardata.car.car_number}}</span>
						</div>
						<div class="xia_1_3_1_2">
							使用性质：<span>{{cardata.car.cheliangxingzhi}}</span>
						</div>
					</div>
					<div class="xia_1_3_1">
						<div class="xia_1_3_1_1">
							变速箱形式：<span>{{cardata.car.huandangfangshi}}</span>
						</div>
						<div class="xia_1_3_1_2">
							排量：<span>{{cardata.car.pailiang}}</span>
						</div>
					</div>
					<div class="xia_1_3_1">
						<div class="xia_1_3_1_1">
							表显里程：<span>{{cardata.car.mileage_number}}</span>
						</div>
						<div class="xia_1_3_1_2">
							上牌日期：<span>{{cardata.car.get_card_time}}</span>
						</div>
					</div>
					<div class="xia_1_3_1">
						<div class="xia_1_3_1_1">
							年检有效期：<span>{{cardata.car.nianjiandaoqi}}</span>
						</div>
						<div class="xia_1_3_1_2">
							保险到期：<span>{{cardata.car.jiaoqiangxiandaoqishijian}}</span>
						</div>
					</div>
				</div>
			</div>
...
<script>
	export default {
		...
		methods: {
			...
			test(){
				// this.cardata.car.max_cost=30000900				
				// this.chulijiage()
			...
			chulijiage() {
				...
				if (this.cardata.car.max_cost >= baoliu * 0.9 && this.cardata.car.max_cost <= baoliu) {
					...
					this.baotext = "超过保留价"
					this.$refs.jiage.style.color = "#ff7e00"
					this.$refs.jiage2.style.color = "#ff7e00"
					this.isbao = true
				} else {
					// console.log(this.$refs.jiage2);3.5vw
					this.$refs.jiage.style.color = "#d92e24"
					this.$refs.jiage2.style.color = "#d92e24"
					this.isbao = false
				}
				...
				if (this.cardata.car.max_cost >= 10 && this.cardata.car.max_cost < 100) {
					...
					this.$refs.jiage.style.fontSize = "9vw"
					this.$refs.mob.style.fontSize = "3.5vw"
				} else if (this.cardata.car.max_cost >= 100 && this.cardata.car.max_cost < 1000) {
					// console.log('>= 100');
					this.$refs.jiage.style.fontSize = "9vw"
					this.$refs.mob.style.fontSize = "3.5vw"
				} else if(this.cardata.car.max_cost >= 1000){
					// console.log('>= 1000');
					this.$refs.jiage.style.fontSize = "9vw"
					this.$refs.mob.style.fontSize = "2vw"
			...
			websocketonmessage(e) { //数据接收
				...
				if (redata.type == 0) {
					if (redata.client_id) {
						...
						this.mobile2 = redata.mobile2
						this.mobile3 = redata.mobile3
						this.chulijiage()
...
<style lang="less" scoped>
	...
	.title_n {
		...
		background: url(../assets/xcp/xcp_02.gif) no-repeat;
