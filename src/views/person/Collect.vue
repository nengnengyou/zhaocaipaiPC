			...
			<el-tab-pane label="竞拍收藏" name="pai">
				<div class="prolist">
					<div class="inner">
						...
						<product-big @click.native="toDetail(item.auction_car_id,1)" class="pro-item" v-for="item in favList.list" :key="item.auction_car_id"
						 :isPai="false" :imgsrc="item.image" :title="item.title" :subtitle="item.catid_text+'丨'+item.car_number+'丨'+item.status_text"
						 :price="item.start_price+'元'"></product-big>
					</div>
				</div>

				<el-pagination  background :page-size="20" @current-change="handleCurrentChange" layout="prev, pager, next"
