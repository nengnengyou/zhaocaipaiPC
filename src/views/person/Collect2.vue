	...
	<div id="person-collect">
		...
		<el-tabs v-model="activeName" @tab-click="handleClick">
			<el-tab-pane label="竞拍收藏" name="pai">
				<div class="prolist">
					<div class="inner">
						<product-big @click.native="toDetail(item.id,1)" class="pro-item" v-for="item in favList" :key="item.id"
						 :isPai="false" :imgsrc="item.image" :title="item.title" :subtitle="$moment(item.end_time * 1000).format('YYYY')+'丨'+item.catid_text+'丨'+item.car_number"
						 :price="item.start_price+'元'"></product-big>
					</div>
				</div>

				<el-pagination :hide-on-single-page="value" background page-size="20" @current-change="handleCurrentChange" layout="prev, pager, next"
				 :total="this.favList.total">
				</el-pagination>
			</el-tab-pane>
