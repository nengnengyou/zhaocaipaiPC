<template>
	<div class="auction-home" ref='auction_home'>
		...
		<!-- 协议 -->
		<el-dialog :close-on-click-modal="false" :close-on-press-escape="false" title="" :visible.sync="dialogVisible" width="50%">
		...
		<div class="auctionhome">
			...
			<div class="auctionhome_left">
				...
				<div class="auctionhome_left_top">
					...
					<div class="auctionhome_left_top1" style=" font-family: '宋体';font-weight: 700; ">
						...
						<el-radio style="margin: 0;background-color: #fff; " v-model="radio2" label="0" border size="medium" @change="getResources()">全部</el-radio>
						<el-radio v-for="item in carfenlei" style="margin: 0;background-color: #fff; margin-left: 5px;" v-model="radio2" :label="item.id" border size="medium"
						 @change="getResources(item.id)">{{item.name}}</el-radio>
					...
					<div class="auctionhome_left_top3">
						...
						<div class="ahsearch">
							<div class="el-input el-input--suffix" style="width:540px;">
								<input type="text" autocomplete="off" placeholder="输入公司名称、标题" class="el-input__inner" v-model='searchtext'>
								<span @click="getResources(null,searchtext)" class="el-input__suffix"><span class="el-input__suffix-inner">
										<i class="el-input__icon el-icon-search"></i>
									</span>
								</span>
							</div>
							<el-button type="primary" style="width: 99px;;" @click="getResources(null,searchtext,true)">查询</el-button>
							<el-button type="primary" style="margin-left: 57px;width: 144px; background-color: #ffa509;" @click='goRecord'>竞拍记录</el-button>
						</div>
				...
				<div class="auctionhome_left_zhong" style="width: 889px; margin-top: 11px;">
					<div class="sort-bar">

						<div class="sort-orders">
							<div>
								<a @click="isd=true;qiehuan('2')" :class="{isdescending:isd}" class="descending">正在竞拍</a>
								<a @click="isd=false;qiehuan('')" :class="{isdescending:!isd}" class="descending">全部竞拍</a>
							</div>
						</div>
					</div>
					...
					<div class="left_nuv_item" v-loading="loading">
						<div v-if="num==0" class="sousuokong">
							<span>
								对不起，没有你搜索的内容! 
								<br>
								<span style="font-size: 18px; color: #da002b;cursor: pointer;text-decoration: underline;"  @click="isd=false;qiehuan('')">{{isd?'去全部竞拍看看>>':''}}</span>
							</span>
						</div>
						<div class="left_nuv_Auction_content">							
							...
							<div class="Auction_content_item" v-for="item in package" :key="item.id" @click="tiaozhuan2(item.id,item.start_time)">
								...
								<div class="Auction_content_item_left">
									...
									<div v-if="!isend(item.end_time)" style="background-color:#919191; height: 60px; line-height: 60px; float: left; text-align: center; padding: 0px 10px;">
										<span class="side-text">已结束</span>

									</div>

									<div v-if="isend(item.end_time) && !isend(item.start_time)" style="background-color:#ff9600; height: 60px; line-height: 60px; float: left; text-align: center; padding: 0px 10px;">
										<span class="side-text">竞拍中</span>
									</div>

									<div v-if="isend(item.end_time) && isend(item.start_time)" style="width: 75px;background-color:#008060; height: 60px; line-height: 60px; float: left; text-align: center; padding: 0px 10px;">
								...
								<div class="Auction_content_item_right">
									<div style="float: left;">
										...
										<div class="item-store_2">
											...
											<div>
												<span class="item-text" style="padding: 0;">{{item.is_pack=="1"?"打包":"单台"}}{{item.status_text}}{{item.is_vip=="1"?" 会员拍":""}}&nbsp;&nbsp;{{item.second==1?"二轮竞拍":""}}</span>
											</div>
									...
									<div style="background-color:#0f94fa; height: 60px; line-height: 60px; float: right; text-align: center; padding: 0px 10px;">
									...
									<div style="padding-top: 10px; padding-left: 8px; float: left;display: flex;">

										<div style="width: 138px;">
											<span class="item-text">{{item.company}}</span>
										</div>
										<div style="position: relative;width: 408px;">
											<div class="item-text" style="position: absolute;right: 0px;">结束时间:
				...
				<div v-if="!num==0">
					<el-pagination @current-change="handleCurrentChange" :current-page.sync="currentPage1" :page-size="20" layout="prev, pager, next, jumper"
					 :total="this.num" style="margin: 0 auto; width: 566px;">
					</el-pagination>
			...
			<div class="auctionhome_right">
				...
				<div class="auctionhome_right_top">
					...
					<div class="right_top_zuo zzz" @click="souc =true">
						<b class="xuanzhongb" :style="souc?'border-bottom: 3px solid #1f9bde;':''">我的收藏</b> <!-- :class="{you_active:souc}" -->
					
					</div>
					
					
					<div class="right_top_you zzz"  @click="souc =false ;">
						<b class="xuanzhongb"  :style="!souc?'border-bottom: 3px solid #1f9bde;':''">我的提醒</b><!-- :class="{you_active:!souc} -->
				...
				<div class="auctionhome_right_txt" style="width: 300px;position: relative; background-color: rgb(251, 249, 249);">

					<div class="right_top_sc" v-if="souc">
						
						<div v-if="souchang.length==0" style="width: 161px; margin: 10px auto 0 auto;; padding: 159px 0;">
							<span style="    font-family: cursive;">收藏列表空空如也~~~</span>
						</div>

						<div class="right_top_sc_item" v-for="item in souchang" :key="item.auction_car_id">

							<div class="sc_item_img" style="float: left;">
								<img v-if="item.image" style="width: 100%;height: 100%;" :src="item.image" alt="">
								<img v-if="!item.image" style="width: 100%;height: 100%;" src="../assets/images/zanwutupian.png" alt="">
							</div>
							<div class="sc_item_txt" style="float: left;width: 192px;margin-left: 5px;">
								<h5 style="margin: 0;"> <a @click="goauction(item.auction_car_id)" class="sc_item_txt_a">{{item.title}}</a></h5>
								...
								<p style="font-size: 12px; overflow: hidden; margin-left: 12px; position: absolute;    bottom: 7px;">{{item.catid_text}}|{{item.car_number}}|{{item.status_text}}</p>

							</div>


						</div>
						<div v-if="souchang.length!=0&&scgd" style="height: 20px;"></div>
						<div v-if="souchang.length!=0&&scgd" class="gengduo" @click="goCollection"> 更多>></div>
					</div>
