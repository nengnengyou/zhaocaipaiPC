<template>
  <div
    class="auc"
    ref="auc"
    style="margin: 0 auto 0 auto; min-width: 1200px; width: 100%; margin-bottom: 15px;"
  >
    <!-- 进入批次前弹窗协议 -->
        ...
        <div class="sort-orders sort-orders2">
          <div class="sou">
            <div class="sou_card">
              <div
                class="card_bt"
                style="padding: 11px 5px;color: #fff;background: #a24e1d;"
              >
                <span
                  style="font: 18px/1.5 tahoma,arial,'Hiragino Sans GB','\5b8b\4f53',sans-serif;"
                  >{{ cardtxt.title }}</span
                >
              </div>
              ...
              <div style="padding-left: 10px;;">
                <div
                  class="card_sl"
                  style="position: absolute;bottom: 100px;font-size: 16px;"
                >
                  竞拍方式:
                  <span style="font-size: 18px; color: rgb(127, 0, 25);"
                    >{{ cardtxt.is_pack == "1" ? "打包拍" : "单台" }}&nbsp;{{
                      cardtxt.is_vip == "1" ? "会员拍" : ""
                    }}&nbsp;{{
                      cardtxt.session_status == "1"
                        ? "实时拍"
                        : cardtxt.session_status == "2"
                        ? " 暗拍"
                        : "明拍"
                    }}</span
                  >
                ...
                <div
                  class="card_sl"
                  style="position: absolute;bottom: 63px;font-size: 16px;"
                >
                  竞拍车辆:
                  <span style="font-size: 18px; color: #7f0019;">{{
                    cardtxt.car_num
                  }}</span>
                  辆
                </div>
                ...
                <div
                  class="card_gs"
                  style="position: absolute;bottom: 5px;font-size: 16px;"
                >
                  <span
                    style="font-size: 18px; margin-left: 10px; color: #7f0019;"
                    >{{ cardtxt.company }}</span
                  >
                </div>
    ...
    <div class="sort-bar-wrap" data-spm="albumSort">
      <div class="sort-bar">
        <div
          class="sort-orders"
          style="display: flex; padding-left: 36px;justify-content: space-between;"
        >
          <div style="display: flex;">
            ...
            <div style="line-height: 40px;width: 230px;">
              <input
                class="inputsousuo"
                v-model="formInline.carNum_or_carVin"
                placeholder="输入标题、车牌号、车架号"
              />
            </div>
            <div
              class="sousuoanniu"
              @click="getcarlist(formInline.carNum_or_carVin)"
            >
              <span>搜索</span>
            </div>
          ...
          <div>
            ...
            <a class="descending" @click="goBulk" style="color: rgb(234, 5, 5);"
              >批量报价</a
            >
          </div>
        </div>
        ...
        <div class="selset-area-box">
          ...
          >
            <el-badge :value="item.count" class="item">
              <el-radio-group
                v-model="radio1"
                @input="getAreaData(item.car_place_info)"
              >
    ...
    <div class="main" ref="main">
      <div class="main_content" v-loading="loading">
        ...
        <div
          ...
          class="main_item"
          :style="
            carlist[index].session_status == '1'
              ? 'min-height: 348px;'
              : 'height: 348px;'
          "
        ...
        >
          ...
          <div class="jiaobiaotitle" v-if="item.is_pack == '1'">
            包{{ item.pack }}
          </div>
          <el-row>
            <el-col :span="1" style="width: 275px;">
              ...
              >
                <div style="width: 100%;height: 202px;    background: #fff;">
                  <img
                    v-if="carlist[index].car_pic_url"
                    :src="
                      carlist[index].car_pic_url +
                        '?x-oss-process=image/resize,m_fixed,h_202,w_275'
                    "
                    class="image"
                    alt=""
                  />
                  ...
                  <img
                    v-if="
                      carlist[index].car_pic_url == '' &&
                        carlist[index].check_report_path == ''
                    "
                    src="../assets/images/无图无检.jpg"
                    class="image2"
                    alt="暂无图片"
                  />
                  <img
                ...
                >
                  <span
                    style="display: block; width: 250px; height: 38px; overflow: hidden;"
                    >{{ carlist[index].title }}</span
                  >

                  <span
                    style="display: block;width: 250px;height: 20px;overflow: hidden;margin-top: 4px;"
                    >{{
                      carlist[index].car_number.substring(
                        0,
                        carlist[index].car_number.length - 3
                      ) + "***"
                    }}|{{ carlist[index].car_place_info }}</span
                  >
                  ...
                  <div class="bottom clearfix" style="bottom: 5px;">
                    ...
                    <div class="Starting_price">
                      {{
                        ...
                        >¥{{
                          cardtxt.status != "2"
                            ? carlist[index].high_money == "0.00"
                              ? carlist[index].start_price
                              : carlist[index].high_money
                            : carlist[index].start_price
                        }}</span
                      >
                    </div>
                    ...
                    <div
                      ...
                      v-if="carlist[index].session_status == '1'"
                    >
                      {{
                        parseInt(carlist[index].high_money) >
                        parseInt(carlist[index].keep_price)
                          ? "已超过保留价"
                          : parseInt(carlist[index].keep_price) * 0.95 <=
                            parseInt(carlist[index].high_money)
                          ? "已临近保留价"
                          : ""
                      }}
                    ...
                    >
                      ...
                      <span style="color: #800019;">{{ time.d }}</span
                      >天 <span style="color: #800019;">{{ time.h }}</span
                      >时 <span style="color: #800019;">{{ time.m }}</span
                      >分 <span style="color: #800019;">{{ time.s }}</span
                      >秒
                    </div>
                  </div>
    ...
    <div class="fenye">
      ...
      >
        <el-pagination
          :background="true"
          style="margin: 0px auto;width: 480px;"
...
export default {
  ...
  methods: {
    ...
    getAreaData(item) {
      ...
      (this.formInline.carNum_or_carVi = ""), (this.p = "");
      this.radio2 = "";
    },
  },
};
