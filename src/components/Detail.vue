<template>
	<div class="detail">
		...
		<el-dialog title="确认订单" v-if="centerDialogVisible" :visible.sync="centerDialogVisible" :before-close="close" width="55%">
			<out-modal :dingdan="dingdan" :goodid="$route.query.id" :spec="spec" :goodCount="goodCount" @close="close"></out-modal>
		</el-dialog>
		<!-- 查询维保 弹出框 -->
		<Query :title="info.title" :plate="info.plate" :type="info.type" :engine="info.engine" :frame="info.frame" @close="dialogVisible = false"
		...
		<div class="detail-inner">
			<div class="detail-top">
						...
						<swiper :options="swiperOption">
							<swiper-slide v-for="(item,index) in buycarInfo.img" :key="index">
								<div class="small-inner">
									<img class="small-img-self" :class="{ active:index == num }" :src="item" alt @click="imgPlus(index)" />
								</div>
							</swiper-slide>
						</swiper>
					...
					<div v-if="isType=='pai'" class="smallPic">
						<swiper :options="swiperOption">
							<swiper-slide v-for="(item,index) in carInfo.car_pic_url" :key="index">
								<div class="small-inner">
									<img class="small-img-self" :class="{ active:index == num }" :src="item" alt style="object-fit: contain"
									 @click="imgPlus(index)" />
								</div>
							</swiper-slide>
						</swiper>
					...
					<div v-if="isType=='zhao'" class="smallPic">
						<swiper :options="swiperOption">
							<swiper-slide v-for="(item,index) in goodInfo.banner" :key="index">
								<div class="small-inner">
									<img class="small-img-self" :class="{ active:index == num }" :src="item" alt @click="imgPlusZhao(index)" />
								</div>
							</swiper-slide>
						</swiper>
						<div class="swiper-button-prev"></div>
				...
				<div class="detile-right" :style="isType == 'buy'?{'padding-bottom':'90px'}:{'padding-bottom':'0px'}">
					...
					<div class="option">
						...
						<button v-if="!carInfo.is_collection&&isType == 'pai'" @click="collection(1)" class="option-btn">收藏</button>
						<button v-if="carInfo.is_collection&&isType == 'pai'" @click="collection(0)" class="option-btn">取消收藏</button>
						<button v-if="!buycarInfo.like_num&&isType == 'buy'" @click="collectionBuy(1)" class="option-btn">收藏</button>
						<button v-if="buycarInfo.like_num&&isType == 'buy'" @click="collectionBuy(0)" class="option-btn">取消收藏</button>
					...
					<div v-if="isType=='zhao'" class="fenlei" :key="index">
						...
						<div class="fenlei-item">
							...
							<el-radio-group v-model="spec" size="mini">
								...
								<el-radio style="line-height: 10px;" v-if="specs[0]==''" label="官方标准" border></el-radio>

								<el-radio style="line-height: 10px;" v-if="!(specs[0]=='')" :key="index" v-for="(item,index) in specs" :label="item"
								 border></el-radio>
							</el-radio-group>
						</div>
					</div>
            ...
            <span class="price-con">
              ...
              <span style="font-size:18px;">元</span>
					...
					<div v-if="isType=='zhao'" class="price">
						<span class="price-title">邮费：</span>&nbsp;&nbsp;&nbsp;<span style="font-size: 18px;"> {{goodInfo.postage}}元</span>
					</div>
					<div v-if="isType=='pai'" class="price">
					...
					<div style="width: 100%">
						...
						<a target="_blank" v-if="(isType=='pai' && carInfo.check_report_path != '')" :href="carInfo.check_report_path"
						 class="chujia-button report-path" style="text-align: center;display: inline-block;margin-left: 10px">查看检验报告</a>
					</div>
					...
					<div v-if="isType=='buy'" class="store-info">
						...
						<router-link :to="'/buystoredetail?id='+buycarInfo.shop_info.id">{{buycarInfo.shop_info.shop_name}}</router-link>
					</div>
				</div>
			</div>
			...
			<div class="detail-content">
				...
				<div v-if="isType=='pai'" class="dc-hart" id="images">
					<img v-for="(item,index) in carInfo.car_pic_url" :key="index" :src="item" style="width:100%;margin-bottom:15px"
					 @click="showBigImg(item, index)" alt />
				</div>
			</div>
		</div>
		<!-- shuaxindom -->
				...
				if (this.spec.length == 0) {
					...
					this.$api.confirm({
						...
						if (res.msg == "库存不足") {
							...
							this.dingdan.length=0
							console.log(res.data)
							this.dingdan.push(res.data)
